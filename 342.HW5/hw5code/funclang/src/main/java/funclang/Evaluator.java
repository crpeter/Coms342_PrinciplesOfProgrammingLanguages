...
public class Evaluator implements Visitor<Value> {
	...
	public Value visit(LessExp e, Env env) {
		//System.out.println("class " + e.first_exp().getClass());
		Value first;
		Value second;
		switch (e.first_exp().getClass().toString()) {
			case "class funclang.AST$StrExp":
				first = (Value.StringVal) e.first_exp().accept(this, env);
				second = (Value.StringVal) e.second_exp().accept(this, env);
				return new Value.BoolVal(first.tostring().length() < second.tostring().length());
			case "class funclang.AST$ListExp":
				first = (Value.PairVal) e.first_exp().accept(this, env);
				second = (Value.PairVal) e.second_exp().accept(this, env);
				//System.out.println("f1: " + ((PairVal) first).fst().tostring() + " f2: " + ((PairVal) first).snd().tostring() +
						//" s1: " + ((PairVal) second).fst().tostring() + " s2: " + ((PairVal) second).snd().tostring());
				if (((PairVal) first)._fst.tostring().isEmpty() && ((PairVal) second)._fst.tostring().isEmpty()) {
					return new Value.BoolVal(false);
				} else if (((PairVal) first)._fst.tostring().isEmpty()) {
					return new Value.BoolVal(false);
				} else if (((PairVal) second)._fst.tostring().isEmpty()) {
					return new Value.BoolVal(true);
				} else {
					return new Value.BoolVal(((PairVal) first).snd().tostring().length() < ((PairVal) second).snd().tostring().length());
				}
			case "class funclang.AST$NumExp":
				first = (Value.NumVal) e.first_exp().accept(this, env);
				second = (Value.NumVal) e.second_exp().accept(this, env);
				return new Value.BoolVal(((NumVal) first).v() < ((NumVal) second).v());
			default:
				return null;
		}
	}
	...
	public Value visit(EqualExp e, Env env) {
		...
		switch (e.first_exp().getClass().toString()) {
			...
			case "class funclang.AST$ListExp":
				first = (Value.PairVal) e.first_exp().accept(this, env);
				second = (Value.PairVal) e.second_exp().accept(this, env);
				return new Value.BoolVal(((PairVal) first).fst().tostring().equals(((PairVal) second).fst().tostring())
											&& ((PairVal) first).snd().tostring().equals(((PairVal) second).snd().tostring()));
			case "class funclang.AST$CarExp":
				first = (Value.PairVal) e.first_exp().accept(this, env);
				second = (Value.PairVal) e.second_exp().accept(this, env);
				return new Value.BoolVal(((PairVal) first).fst().tostring().equals(((PairVal) second).fst().tostring())
						&& ((PairVal) first).snd().tostring().equals(((PairVal) second).snd().tostring()));
			case "class funclang.AST$CdrExp":
				first = (Value.PairVal) e.first_exp().accept(this, env);
				second = (Value.PairVal) e.second_exp().accept(this, env);
				return new Value.BoolVal(((PairVal) first).fst().tostring().equals(((PairVal) second).fst().tostring())
						&& ((PairVal) first).snd().tostring().equals(((PairVal) second).snd().tostring()));
			case "class funclang.AST$ConsExp":
				first = (Value.PairVal) e.first_exp().accept(this, env);
				second = (Value.PairVal) e.second_exp().accept(this, env);
				return new Value.BoolVal(((PairVal) first).fst().tostring().equals(((PairVal) second).fst().tostring())
						&& ((PairVal) first).snd().tostring().equals(((PairVal) second).snd().tostring()));
	...
	@Override
	public Value visit(NumberExp e, Env env) {
		return new Value.BoolVal(e.getClass().equals(Value.NumVal.class));
	}

	@Override
	public Value visit(BooleanExp e, Env env) {
		return new Value.BoolVal(e.getClass().equals(Value.BoolVal.class));
	}

	@Override
	public Value visit(StringExp e, Env env) {
		return new Value.BoolVal(e.getClass().equals(Value.StringVal.class));
	}

	@Override
	public Value visit(ProcedureExp e, Env env) {
		return new Value.BoolVal(e.getClass().equals(Value.FunVal.class));
	}

	@Override
	public Value visit(List_Exp e, Env env) {
		return new Value.BoolVal(e.getClass().equals(Value.PairVal.class));
	}
