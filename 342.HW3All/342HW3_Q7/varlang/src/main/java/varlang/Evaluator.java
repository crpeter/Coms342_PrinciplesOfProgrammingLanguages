...
public class Evaluator implements Visitor<Value> {
	...
	public Value visit(LetExp e, Env env) { // New for varlang.
		...
		for (int i = 0; i < value_exps.size(); i++) {
			Exp currentVal = value_exps.get(i);
			while (!isNumeric(currentVal.toString())) {
				for (int j = 0; j < names.size(); j++) {
					if (names.get(j).equals(currentVal.toString())) {
						currentVal = value_exps.get(j);
					}
				}
			}
			values.add((Value)currentVal.accept(this, env));
		}
		//System.out.println("names: " + names + "\nvalue_exp: " + value_exps + "\nvalues: " + values);
		Env new_env = env;
		for (int i = 0; i < names.size(); i++) {
			new_env = new ExtendEnv(new_env, names, values);
		}
